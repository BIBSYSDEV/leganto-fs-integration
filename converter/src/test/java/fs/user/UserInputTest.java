...
public class UserInputTest {
    ...
    private static File createTemporaryFile(String filename) throws IOException {
        ...
        File participantsTempFile = Files
            .createTempFile(splitFilename[PREFIX], splitFilename[SUFFIX])
            .toFile();
    ...
    public void getNumberOfParticipantsShouldReturnTheRespectiveEntryFromTheUserFileIfEntryExists()
        ...
        assertThat(userInput.getNumberOfParticipants(RESORUCE_FILE_COURSE_CODE)
                .isPresent(),
            is(equalTo(true)));
        assertThat(userInput.getNumberOfParticipants(RESORUCE_FILE_COURSE_CODE)
            .get(), is(equalTo(
    ...
    public void getNumberOfParticipantsShouldReturnEmptyifTheCourseCodeDoesNotExist()
        ...
        ObjectNode userInputJson = inputWithBothFiles();
        UserInput userInput = readValue(userInputJson, UserInput.class).initFiles();
        assertThat(userInput.getNumberOfParticipants(NON_EXISTING_COURSE_CODE)
                .isPresent(),
            is(equalTo(false)));
    }
    ...
    public void getCampusParticipantsShouldReturnTheRespectiveEntryfromTheUserFileIfEntryExists()
        ...
        ObjectNode userInputJson = inputWithBothFiles();
        UserInput userInput = readValue(userInputJson, UserInput.class).initFiles();
        assertThat(userInput.getCampusParticipants(RESORUCE_FILE_COURSE_CODE)
                .isPresent(),
            is(equalTo(true)));
        assertThat(userInput.getCampusParticipants(RESORUCE_FILE_COURSE_CODE)
            .get(), is(equalTo(
    ...
    public void getCampusParticipantsShouldReturnEmptyifTheCourseCodeDoesNotExist()
        ...
        ObjectNode userInputJson = inputWithBothFiles();
        UserInput userInput = readValue(userInputJson, UserInput.class).initFiles();
        assertThat(userInput.getCampusParticipants(NON_EXISTING_COURSE_CODE)
                .isPresent(),
            is(equalTo(false)));
    }
    ...
    public void fieldsShouldNotBePrimitive()
        ...
        List<Method> getters = Arrays.asList(userInput.getClass()
            .getDeclaredMethods())
            .stream()
        ...
        for (Method getter : getters) {
            ...
            boolean isNotPrimitive = Objects.nonNull(result) && !result.getClass()
                .isPrimitive();

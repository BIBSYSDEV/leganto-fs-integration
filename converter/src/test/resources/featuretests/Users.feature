Feature: Instructors for each Leganto entry
  ...
  Background:
    ...
    And the user input has a field with value "rolle_codes" that is an array with values
      | rolle1 |
      | rolle2 |
      | rolle3 |

    And the user input has a field with value "ims_codes" that is an array with values
      | ims1 |
      | ims2 |
      | ims3 |
